...
for i in range(len(query_choose_dims)):
 sql_final = f"""
            TRUNCATE TABLE {database_cube_name}.{cubes_table[i]};
            INSERT INTO {database_cube_name}.{cubes_table[i]}
...
output = f"""
     CREATE PROCEDURE ProcessNewValuesFact_sales
     AS
     BEGIN 
     {'\n'.join(sql_results)}
     END;
"""
output_file = "output_procedure_insert_cube.txt"
exportSQL([output],output_file)